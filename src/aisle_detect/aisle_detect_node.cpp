...
void AisleDetectNode::onInit()
{
    ...
    pub_line_ = nh.advertise<sensor_msgs::PointCloud2>("cluster_line", 10);
    pub_points_ = nh.advertise<sensor_msgs::PointCloud2> ("points_msg", 10);
...
void AisleDetectNode::scanCallback(const sensor_msgs::LaserScan::ConstPtr& scan_msg)
{
    ...
    this->projector_.projectLaser(*scan_msg, cloud_temp);
    ...
    pcl::PointXYZ reference (sum_x / (float)num_points, (line_start_y + line_end_y)/2, 0);
    ...
    point_set.push_back(line_start_point); // point_set[2]
    point_set.push_back(line_end_point); // point_set[3]
